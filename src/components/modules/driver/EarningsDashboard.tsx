...
// console.log("earni", earningsData)
  ...
  const processedData = useMemo<IEarningsGroups>(() => {
    ...
    const dailyEarnings = rides.reduce((acc: any, ride) => {
      const date = new Date(ride.rideTimestamps.acceptedAt).toLocaleDateString();
      if (!acc[date]) {
    ...
    const weeklyEarnings = rides.reduce((acc: any, ride) => {
      const date = new Date(ride.rideTimestamps.completedAt);
